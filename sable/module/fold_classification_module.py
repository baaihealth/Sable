...
class FoldClassificationModule(LightningModule):
    def __init__(
        self,
        loss: nn.Module,
        model: nn.Module,
        ...
        **kwargs,
        ...
        super(FoldClassificationModule, self).__init__()

        self.loss = loss
        self.model = model
        self.metrics = [{ "accuracy": MulticlassAccuracy(1195, average='micro') } for _ in range(2)]
    ...
    def summarize_test_results(self, test_idx: int):
        ...
        rank_zero_info("\n====Testing Results for Fold Classification on \"%s\" with %d Cases====" % (
