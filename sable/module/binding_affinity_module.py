...
class BindingAffinityModule(LightningModule):
    def __init__(
        self,
        loss: nn.Module,
        model: nn.Module,
        ...
        **kwargs,
        ...
        """
        Initialize a `BindingAffinityModule`.
        ...
        super(BindingAffinityModule, self).__init__()

        self.loss = loss
        self.model = model
        self.metrics = [{ "RMSE": MeanSquaredError(squared=True), "Pearson": PearsonCorrCoef() } for _ in range(2)]
    ...
    def validation_step(self, batch: Mapping[str, Tensor]) -> Tensor:
        ...
        for m in self.metrics[0].values():
            m.update(loss_breakdown["pred_ddg"], loss_breakdown["tgt_ddg"])

        return loss_breakdown["loss"]

    def on_validation_epoch_end(self):
        self.log("valid/pearson", self.metrics[0]["Pearson"].compute(), on_step=False, on_epoch=True, logger=self.trainer.logger, sync_dist=True)
        self.log("valid/rmse", self.metrics[0]["RMSE"].compute(), on_step=False, on_epoch=True, logger=self.trainer.logger, sync_dist=True)
        for m in self.metrics[0].values():
            m.reset()
    ...
    def summarize_test_results(self, test_idx: int):
        ...
        rank_zero_info("\n====Testing Results for Binding Affinity on \"%s\" with %d Cases====" % (
        ...
        rank_zero_info("+" + (("-" * cw) + "+") * 2)
        rank_zero_info("|" + (" " * ((cw - 2) // 2)) + "Rp" + (" " * ((cw - 2 + 1) // 2)) + "|" + (" " * ((cw - 4) // 2)) + "RMSE" + (" " * ((cw - 4 + 1) // 2)) + "|") # detailed header
        rank_zero_info("+" + (("-" * cw) + "+") * 2)
        value_format_str = " %%%d.6f |" % (cw - 2)
        rank_zero_info(("|" + value_format_str * 2) % (self.metrics[1]["Pearson"].compute(), self.metrics[1]["RMSE"].compute())) # detailed metrics
        rank_zero_info("+" + (("-" * cw) + "+") * 2)
