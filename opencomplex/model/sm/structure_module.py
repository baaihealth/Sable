    ...
    def __init__(
        ...
        **kwargs,
        ...
        if kwargs["share_weights"]:
            ...
            self.ipa = nn.ModuleList(InvariantPointAttention(
                self.c_s,
                self.c_z,
                self.c_ipa,
                self.no_heads_ipa,
            ...
            self.ipa_dropout = nn.ModuleList(nn.Dropout(self.dropout_rate) for i in range(self.no_blocks))
            self.layer_norm_ipa = nn.ModuleList(LayerNorm(self.c_s) for i in range(self.no_blocks))

            self.transition = nn.ModuleList(StructureModuleTransition(
                self.c_s,
                self.no_transition_layers,
                self.dropout_rate,
            ) for i in range(self.no_blocks))
